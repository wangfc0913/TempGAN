...
You need to specify the IRImages ('--dataroot'), experiment name ('--name'), and model ('--model').
It first creates model, IRImages, and visualizer given the option.
...
if __name__ == '__main__':
    ...
    dataset_size = len(dataset)    # get the number of images in the IRImages.
    ...
    for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):    # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
        ...
        for i, data in enumerate(dataset):  # inner loop within one epoch
            ...
            model.set_input(data)         # unpack data from IRImages and apply preprocessing
